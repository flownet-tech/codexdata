...
describe("/v1/features/codex", () => {
  it("serves the merged registry at latest.json with ETag + 304", async () => {
    ...
    expect(etag).toMatch(/^"features-[0-9a-f]{32}"$/);

    const body = (await res.json()) as Payload;
    expect(body.dataset).toBe("codex-feature-flags");
    expect(body.snapshot_tag).toBe(tags.snapshot_aliases[tags.latest as never] ?? tags.latest);
    const latestSnapshot = tags.snapshot_aliases[tags.latest as never] ?? tags.latest;
  ...
  it("serves every verified tag; alias tags share the snapshot body and ETag", async () => {
    let aliasEtag: string | null = null;
    ...
    for (const tag of tags.verified_tags) {
      ...
      if (tag === "rust-v0.153.1") aliasEtag = res.headers.get("etag");
      if (tag === "rust-v0.153.4") snapshotEtag = res.headers.get("etag");
    }
    expect(aliasEtag).toBe(snapshotEtag);
